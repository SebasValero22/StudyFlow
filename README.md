...
**StudyFlow** is a modern, production-grade academic management ecosystem designed to help students streamline their academic life. Instead of relying on generic tools, StudyFlow is custom-tailored to the student workflow — centralizing modules (subjects), assignments, exam schedules, and automatically calculating weighted averages.
...
- **Strict Status Tracking:** Tasks are tracked with natural state labels (`✓ HECHA` / `✗ SIN HACER`) in green and gray, instead of confusing raw boolean values.
...
## 🔐 Security & Reliability Polish (Ver. 2.0)
...
| **subjects** | `subjectId` (INT) | `userId` → **users** | `nameSubject`, `color`, `academicYear`, `activeSubject` |
| **tasks** | `taskId` (INT) | `subjectId` → **subjects** | `title`, `descriptionTask`, `due_date`, `isCompleted`, `priority` |
| **exams** | `examId` (INT) | `subjectId` → **subjects** | `nameExam`, `examType`, `examDate`, `classroom` |
| **grades** | `gradeId` (INT) | `subjectId` → **subjects** | `score` (DOUBLE), `weight` (DOUBLE) |
...
- *Course:* 2º DAM (Development of Multiplatform Applications)
...
The PWA is automatically built and deployed via Netlify using the `netlify.toml` file at the root of the repository, enabling seamless monorepo deployments directly from Git pushes.
