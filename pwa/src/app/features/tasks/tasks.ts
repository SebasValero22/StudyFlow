...
export class TasksComponent implements OnInit {
  ...
  toggleComplete(task: Task) {
    if (!task.taskId || this.loading) return;
    this.loading = true;
    ...
    this.api.updateTask(task.taskId, task).subscribe({
      next: () => {
        this.loading = false;
        // No llamamos a loadData aqui para evitar parpadeo; el estado local ya esta actualizado.
      },
      error: (err) => {
        this.loading = false;
        task.isCompleted = previousState; // Revertir si hay error
